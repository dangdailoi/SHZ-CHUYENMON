...
const KPICard = ({ icon:Icon, label, value, gap, color="zinc", insight }) => {
  const cls = { rose:"text-rose-400 bg-rose-500/10", blue:"text-blue-400 bg-blue-500/10",
    emerald:"text-emerald-400 bg-emerald-500/10", amber:"text-amber-400 bg-amber-500/10",
    violet:"text-violet-400 bg-violet-500/10", zinc:"text-zinc-400 bg-zinc-500/10" };
  return (
    <div className="group relative">
      <div className="flex items-center gap-3 p-4 bg-zinc-900/60 rounded-xl border border-zinc-800/50 hover:border-zinc-700 transition-all">
        <div className={`p-2.5 rounded-lg ${cls[color]}`}><Icon className="w-4 h-4" /></div>
...
const InsightCard = ({ icon, title, value, description, type="default" }) => {
  const s = { opportunity:"border-amber-500/30 bg-gradient-to-br from-amber-500/10 to-transparent",
  ...
  const t = { opportunity:"text-amber-400", success:"text-emerald-400", warning:"text-rose-400", default:"text-zinc-300" };
  return (
    <div className={`p-5 rounded-xl border ${s[type]}`}>
      <div className="flex items-start gap-3">
        <span className="text-2xl">{icon}</span>
        <div className="flex-1">
          <div className="flex items-center justify-between">
            <span className="text-sm text-zinc-400">{title}</span>
            <span className={`text-2xl font-bold ${t[type]}`}>{value}</span>
          </div>
...
const ExpandableSection = ({ title, children, defaultOpen=false, badge }) => {
  ...
  return (
    <div className="border border-zinc-800/50 rounded-xl overflow-hidden">
      ...
      {open && <div className="p-4 bg-zinc-900/20">{children}</div>}
    </div>
  );
};
    ...
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      <header className="sticky top-0 z-50 bg-zinc-950/90 backdrop-blur-sm border-b border-zinc-800/50">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="text-xs text-rose-400 font-medium tracking-wider">STRATEGIC MARKET ANALYSIS</div>
              <h1 className="text-base font-semibold text-white">PHÂN TÍCH THỊ TRƯỜNG ĐÀO TẠO TIẾNG TRUNG 2025</h1>
            </div>
            <nav className="flex gap-1 bg-zinc-900/50 p-1 rounded-lg overflow-x-auto">
              {tabs.map(t => (
                <button key={t.id} onClick={() => setTab(t.id)}
                  ...
                  {t.label}
                  {t.badge && <span className="px-1 py-0.5 text-xs bg-rose-500/20 text-rose-400 rounded">{t.badge}</span>}
                </button>
              ))}
            </nav>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
          ...
          <div className="space-y-8">
            <section className="text-center py-6">
              <div className="flex items-center justify-center gap-12">
                ...
                <div className="text-center">
                  <div className="text-5xl font-bold text-white">{data.totalCourses}</div>
                  <div className="text-sm text-zinc-400 mt-1">Khóa học phân tích</div>
                </div>
              </div>
              <div className="flex items-center justify-center gap-8 mt-8">
                {data.regions.slice(0,2).map((r,i) => (
                  <div key={r.name} className="flex items-center gap-8">
                    <div className="text-center">
                      <div className="text-2xl font-semibold text-white">{r.centers}</div>
                      <div className="text-xs text-zinc-500">{r.name}</div>
                      <div className="text-xs text-zinc-600">TB: {fmt(r.avgPrice)}</div>
                    </div>
            ...
            <section className="bg-gradient-to-br from-rose-500/10 via-transparent to-blue-500/10 rounded-2xl p-6 border border-zinc-800/50">
              ...
              <div className="mt-6 p-4 bg-zinc-900/60 rounded-xl">
                <p className="text-sm text-zinc-400">
                  <strong className="text-amber-400">💡 Insight:</strong> Với việc{" "}
                  <span className="text-rose-400">98%</span> đối thủ bỏ ngỏ phòng thi thử và{" "}
                  <span className="text-rose-400">82%</span> thiếu giáo viên bản xứ, đây là "Đại dương xanh" để bứt phá. Tập trung vào hai mũi nhọn này sẽ chuyển đổi vị thế từ một trung tâm dạy học thuần túy sang{" "}
                  <span className="text-rose-400">Đơn vị bảo chứng kết quả</span>, trực tiếp đánh vào nhu cầu cấp thiết nhất mà thị trường chưa đáp ứng được.
                </p>
              </div>
            </section>
            ...
            <section>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <KPICard icon={Globe}         label="App học tập"         value={1}  gap={99} color="zinc"    insight="GAP lớn nhất thị trường; cơ hội số hóa" />
              </div>
            </section>

            <section className="bg-zinc-900/40 rounded-2xl p-6 border border-zinc-800/50">
              <h2 className="text-lg font-semibold text-white mb-1">Phân bố khóa học</h2>
              <p className="text-sm text-zinc-500 mb-6">HSK 1-3 chiếm 67% • Thiếu nhi có giá cao nhất</p>
              <div className="grid grid-cols-2 gap-6">
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie data={data.courseDistribution} dataKey="count" nameKey="name" cx="50%" cy="50%" outerRadius={80}
                         label={({ name, percent }) => `${name}: ${Math.round(percent*100)}%`}>
                      {data.courseDistribution.map((e,i) => <Cell key={i} fill={e.color} />)}
                    </Pie>
                    <Tooltip />
                  </PieChart>
                </ResponsiveContainer>
            ...
            <section>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <InsightCard icon="📱" title="App học tập"              value="99%" description="99% thị trường bỏ ngỏ công nghệ → Trải nghiệm khác biệt" type="success" />
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-zinc-500 text-xs uppercase border-b border-zinc-800">
                      ...
                      <th className="text-right py-4 px-2 text-blue-400">Online HCM</th>
                      <th className="text-right py-4 px-2 text-emerald-400">Offline HCM</th>
                      <th className="text-right py-4 px-2 text-indigo-400">Online BD</th>
                      <th className="text-right py-4 px-2 text-teal-400">Offline BD</th>
                  ...
                  <tbody className="text-zinc-300">
                    {priceData.map((item,i) => (
                      <tr key={i} className="border-b border-zinc-800/50 hover:bg-zinc-800/30 transition-colors">
                        ...
                        <td className="text-right px-2">{item.onlineHCM.toLocaleString()}</td>
                        <td className="text-right px-2">{item.offlineHCM.toLocaleString()}</td>
                        ...
                        <td className="text-right px-2">
                          <div className="flex flex-col items-end">
                            <span className="text-[10px] text-zinc-500">{item.min.toLocaleString()} - {item.max.toLocaleString()}</span>
                            <div className="w-24 h-1.5 bg-zinc-800 rounded-full mt-1 overflow-hidden">
                              <div className="h-full bg-gradient-to-r from-orange-500 to-rose-500"
            ...
            <section className="bg-zinc-900/40 rounded-2xl p-6 border border-zinc-800/50">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={data.priceByLevel}>
                  ...
                  <YAxis stroke="#71717a" fontSize={12} tickFormatter={v => fmt(v)} />
                  <Tooltip formatter={v => fmt(v)} contentStyle={{ backgroundColor:"#18181b", border:"1px solid #3f3f46" }} />
                ...
                <div className="space-y-3">
                  {data.topCenters.byHighPrice.map((item,i) => (
                    <div key={i} className="flex items-center justify-between p-3 bg-zinc-800/30 rounded-lg">
                      ...
                      <div className="text-rose-400 font-semibold">{fmt(item.avgPrice)}</div>
                    </div>
                  ))}
                </div>
              </section>
                ...
                <div className="space-y-3">
                  {data.topCenters.byLowPrice.map((item,i) => (
                    <div key={i} className="flex items-center justify-between p-3 bg-zinc-800/30 rounded-lg">
                      ...
                      <div className="text-emerald-400 font-semibold">{fmt(item.avgPrice)}</div>
                    </div>
                  ))}
                </div>
              </section>
          ...
          <div className="space-y-8">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-3">
                  {data.curriculum.map((item,i) => (
                    <SimpleBar key={i} label={item.name} value={item.percent}
                               color={i===0?"#3b82f6":i===1?"#f59e0b":"#6b7280"} subLabel={`${item.count} khóa`} />
                  ))}
                </div>
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <PieChart>
                      <Pie data={data.teachers.types} dataKey="percent" nameKey="name" cx="50%" cy="50%" outerRadius={70}>
                        {data.teachers.types.map((e,i) => (
                          <Cell key={i} fill={i===0?"#3b82f6":i===1?"#f59e0b":i===2?"#10b981":"#6b7280"} />
                        ))}
                      </Pie>
                      <Tooltip />
                    </PieChart>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {data.teachers.qualifications.map((item,i) => (
                      <SimpleBar key={i} label={item.name} value={item.percent} maxValue={30}
            ...
            <section className="bg-zinc-900/40 rounded-2xl p-6 border border-zinc-800/50">
              ...
              <div className="mt-4 p-4 bg-zinc-800/30 rounded-xl text-xs text-zinc-400">
                ...
                3 buổi/tuần, 90 phút/buổi (chiếm {">"} 80% thị trường).
                Riêng HSK 5 chuyển sang lộ trình 2 buổi/tuần (89%) để phù hợp yêu cầu tự học.
              </div>
            </section>
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie data={data.equipment} dataKey="percent" nameKey="name" cx="50%" cy="50%" outerRadius={70} label>
                      {data.equipment.map((e,i) => <Cell key={i} fill={e.color} />)}
                    </Pie>
                    <Tooltip />
                  </PieChart>
                </ResponsiveContainer>
        ...
        {tab === "diagnostic" && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 gap-6">
              {data.diagnosticInsights.map((d,i) => <DiagnosticCard key={i} {...d} />)}
            </div>
                ...
                {data.services.map((s,i) => (
                  <div key={i} className="flex items-center gap-4">
                    <span className={`text-xs px-2 py-1 rounded ${
                      s.type==="gap"      ?"bg-rose-500/20 text-rose-400":
                      s.type==="premium"  ?"bg-amber-500/20 text-amber-400":
                      s.type==="value-add"?"bg-blue-500/20 text-blue-400":
                      "bg-zinc-700 text-zinc-400"}`}>
                      {s.type==="gap"?"GAP":s.type==="premium"?"Premium":s.type==="value-add"?"Giá trị+":"Chuẩn"}
                    </span>
                    <span className="text-sm text-zinc-300 w-44">{s.name}</span>
                    <div className="flex-1 h-3 bg-zinc-800 rounded-full overflow-hidden">
                      <div className="h-full rounded-full transition-all"
                           style={{ width:`${s.percent}%`,
                             backgroundColor:s.type==="gap"?"#f43f5e":s.type==="premium"?"#f59e0b":"#3b82f6" }} />
                    </div>
                    <span className="text-sm font-semibold text-white w-12 text-right">{s.percent}%</span>
                    <span className="text-xs text-zinc-500 w-16 text-right">Gap: {100-s.percent}%</span>
                  </div>
                ))}
              </div>
            </section>
          ...
          <div className="space-y-8">
            <section className="bg-zinc-900/40 rounded-2xl p-6 border border-zinc-800/50">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="text-zinc-300">
                    {data.topCenters.byCourses.map((c,i) => (
                      <tr key={i} className="border-b border-zinc-800/50 hover:bg-zinc-800/30">
                        <td className="py-3 text-rose-400 font-bold">{i+1}</td>
                        <td className="py-3 font-medium text-white">{c.name}</td>
                        <td className="text-center">{c.courses}</td>
                        <td className="text-center text-zinc-500">{c.region}</td>
                        <td className="text-right text-amber-400">{fmt(c.avgPrice)}</td>
                        <td className="text-center">{c.hasGVBX   ?<CheckCircle className="w-4 h-4 text-emerald-400 mx-auto"/>:<XCircle className="w-4 h-4 text-zinc-600 mx-auto"/>}</td>
                        <td className="text-center">{c.hasCommitment?<CheckCircle className="w-4 h-4 text-emerald-400 mx-auto"/>:<XCircle className="w-4 h-4 text-zinc-600 mx-auto"/>}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-zinc-500 text-xs uppercase border-b border-zinc-800">
                      <th className="text-left py-3">Trung tâm</th>
                      <th className="text-center py-3">Giá (20%)</th>
                  ...
                  <tbody className="text-zinc-300">
                    {data.competitiveMatrix.map((c,i) => (
                      <tr key={i} className="border-b border-zinc-800/50 hover:bg-zinc-800/30">
                        <td className="py-3 font-medium text-white">{c.name}</td>
                        <td className="text-center">{c.price}/10</td>
                        <td className="text-center">{c.gvbx}/10</td>
                        <td className="text-center">{c.commitment}/10</td>
                        <td className="text-center">{c.csvc}/10</td>
                        <td className="text-center">{c.consult}/10</td>
                        <td className="text-center">{c.diversity}/10</td>
                        <td className="text-center font-bold text-rose-400">{c.total.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <section className="bg-zinc-900/40 rounded-2xl p-6 border border-zinc-800/50">
              ...
              <ResponsiveContainer width="100%" height={350}>
                <RadarChart data={[
                  { subject:"Giá cả",  CGE:6, Newsky:8, THANHMAIHSK:5  },
                  { subject:"GVBX",    CGE:8, Newsky:7, THANHMAIHSK:5  },
                  { subject:"Cam kết", CGE:9, Newsky:8, THANHMAIHSK:9  },
                  { subject:"CSVC",    CGE:9, Newsky:7, THANHMAIHSK:7  },
                  { subject:"Tư vấn",  CGE:6, Newsky:7, THANHMAIHSK:9  },
                  { subject:"Đa dạng", CGE:10,Newsky:8, THANHMAIHSK:6  },
                ]}>
            ...
            <ExpandableSection title="📋 Danh sách 11 trung tâm có GVBX" badge="Khan hiếm">
              <div className="grid grid-cols-3 gap-3">
                {data.teachers.centersWithGVBX.map((name,i) => (
                  <div key={i} className="p-3 bg-amber-500/10 rounded-lg border border-amber-500/20 text-sm text-amber-400">{name}</div>
                ))}
              </div>
            </ExpandableSection>
          </div>
        ...
        {tab === "recommendations" && (
          <div className="space-y-8">
            <section>
              <h2 className="text-sm font-medium text-zinc-500 uppercase tracking-wider mb-4">🚀 Quick Wins (0-3 tháng) — Làm ngay</h2>
            ...
            <section>
              <h2 className="text-sm font-medium text-zinc-500 uppercase tracking-wider mb-4">📈 Short-term (3-6 tháng) — Xây dựng năng lực</h2>
              <div className="grid grid-cols-3 gap-4">
            ...
            <section>
              <h2 className="text-sm font-medium text-zinc-500 uppercase tracking-wider mb-4">🎯 Medium-term (6-12 tháng) — Scale & Differentiate</h2>
              <div className="grid grid-cols-3 gap-4">
                ...
                {[
                  ...
                  <div key={i} className="flex items-center gap-4 p-3 bg-zinc-800/30 rounded-lg">
                    <span className={`px-2 py-1 text-xs font-bold rounded ${
                      ...
                      "bg-amber-500/20 text-amber-400"}`}>{item.priority}</span>
                    <div className="flex-1">
            ...
            <ExpandableSection title="💎 Key Takeaways" badge="Quan trọng" defaultOpen={true}>
              <div className="space-y-4">
                <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl">
                  <div className="text-sm font-medium text-amber-400 mb-2">Cam kết đang trở thành tiêu chuẩn</div>
                  <p className="text-xs text-zinc-400">46% đã có cam kết học lại. Không có cam kết = mất lợi thế cạnh tranh. Điều kiện hợp lý: 85% tham gia + Thi cuối kỳ.</p>
                </div>
                <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl">
                  <div className="text-sm font-medium text-blue-400 mb-2">GVBX là USP khan hiếm</div>
                  <p className="text-xs text-zinc-400">82% không có GVBX. Chỉ cần 4-6 buổi/khóa cho phần Nghe-Nói đã tạo khác biệt lớn. Hợp tác bán thời gian là giải pháp khả thi.</p>
                </div>
              </div>
            </ExpandableSection>
          </div>
      ...
      <footer className="border-t border-zinc-800/50 mt-12">
        <div className="max-w-7xl mx-auto px-6 py-4 text-center">
          <p className="text-xs text-zinc-600">Strategic Market Analysis Dashboard • 62 trung tâm • 179 khóa học • HCM & Bình Dương • Q1/2025</p>
        </div>
      </footer>
    </div>
  );
